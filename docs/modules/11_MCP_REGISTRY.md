...
3. `McpServerDefinition.SecretParamNamesJson` é **declarativo, não é imposto** — hoje nada valida, ao criar/atualizar uma `McpServerAssignment`, que todo nome ali listado realmente aparece em `ParamValuesJson` como `{"secretId": ...}` (nem que apareça de jeito nenhum). Uma assignment pode ser criada faltando um parâmetro exigido, ou pode declarar um valor "obrigatoriamente-secret" como string literal. Gap conhecido, não corrigido nesta revisão.
...
|---|---|---|---|---|---|
| AC-01 | Owner/Admin autenticado | registra uma definition nova via REST ou `admin.mcp.register` | `201`, `AuditLog MCP_SERVER_REGISTERED` | Integration/E2E | **Não coberto por teste dedicado hoje** — só validado manualmente via build+smoke (ver seção 15) |
| AC-02 | identidade sem `McpRegistryWrite` | tenta registrar/remover/assign/revoke | `403`/`forbidden` | Security | **Não coberto por teste dedicado hoje** |
| AC-03 | identidade sem nenhum `RoleAssignment` ativo | Owner tenta criar assignment para ela | `409 identity_has_no_role_assignment` | Integration | **Não coberto por teste dedicado hoje** |
...
| AC-05 | assignment de outra identidade, chamador sem `SecretReadValue` no secret referenciado | chamador tenta renderizar | `403 forbidden`, `AuditLog FAILED_ACCESS`, nenhum vazamento | Security | **Não coberto por teste dedicado hoje** |
...
**Gap real:** ao contrário de todo módulo anterior (`RbacRevealAuditTests`, `RoleAssignmentEndpointTests`, `AuditAndRevokeEndpointTests`), este módulo não tinha nenhum arquivo de teste dedicado em `tests/Security` ou `tests/Integration` — a única cobertura automatizada era a listagem de nomes de tool em `McpToolsTests.cs`. A rev3 adiciona `tests/Security/ForgeVault.Security.Tests/McpAssignmentRenderTests.cs`, mas cobrindo especificamente o gap de `RevokedAt` da seção 12 (revoke→render vira `409`, sem vazamento, com audit). Nenhum dos fluxos AC-01 a AC-03 e AC-05 acima ainda tem teste dedicado.
...
- [ ] testes e evidências definidos — AC-06 e o gap de `RevokedAt` (seção 12) têm teste dedicado (`McpToolsTests.ToolsList_ReturnsAllPlannedTools`, `McpAssignmentRenderTests`); AC-01 a AC-03 e AC-05 ainda não têm cobertura automatizada, e AC-04 só parcialmente (ver seção 13)
- [ ] observabilidade definida — nenhuma métrica/log estruturado além de `AuditLog` (seção 11)
- [ ] nenhuma questão bloqueante aberta — duas abertas (seção 1): script de sync ausente, métricas ausentes. O gap de segurança da seção 12 (`RevokedAt` não checado em render) foi corrigido na rev3 e não bloqueia mais revisões futuras.
