...
decisions:
  ...
  - "M10 rev5: cobertura de teste completada para a tabela de AC da seção 13 — `McpToolsTests.AdminMcpRegisterAssignRevoke_ViaMcp_AreRbacGated_AndEnforceRoleAssignmentInvariant` (E2E) cobre AC-01/AC-02/AC-03 na superfície MCP (`admin.mcp.register/assign/revoke_assignment`), espelhando o que `McpRegistryEndpointTests` já cobria só em REST; `McpAssignmentRenderTests.Render_Self_AuditsSecretReveal_WithoutLeakingValueInTheAuditRow` fecha AC-04 isolado, asserindo o `AuditLog SECRET_REVEAL` diretamente (o teste de `RevokedAt` já exercitava o render com sucesso como setup, mas nunca esse assert). Todas as ACs de AC-01 a AC-06 têm evidência automatizada a partir desta revisão."
  - "M14: `mcp_server_renders_total` (`src/ForgeVault.Api/Observability/McpMetrics.cs`) adicionado, incrementado em toda saída de `McpAssignmentRenderer.RenderAsync` com tag `outcome` — fecha a segunda open_blocking_question desta seção parcialmente (só o counter deste módulo; `mcp_calls_total`, do módulo 09, cobrindo as ~20 tools MCP no geral, continua não implementado). Só `System.Diagnostics.Metrics` da BCL — nenhum exportador (Prometheus/OTLP) existe neste projeto."
...
- métricas: `mcp_server_renders_total` (M14, `src/ForgeVault.Api/Observability/McpMetrics.cs`), tag `outcome` — só `System.Diagnostics.Metrics`, sem exportador configurado. `mcp_calls_total` do módulo 09 (cobriria as tools deste módulo também) continua não implementada.
...
|---|---|---|---|---|---|
| AC-01 | Owner/Admin autenticado | registra uma definition nova via REST ou `admin.mcp.register` | `201`, `AuditLog MCP_SERVER_REGISTERED` | Integration/E2E | **Satisfeito** — REST: `McpRegistryEndpointTests.RegisterDefinition_RequiresMcpRegistryWrite_AndAuditsOnSuccess`; MCP: `McpToolsTests.AdminMcpRegisterAssignRevoke_ViaMcp_AreRbacGated_AndEnforceRoleAssignmentInvariant` |
| AC-02 | identidade sem `McpRegistryWrite` | tenta registrar/remover/assign/revoke | `403`/`forbidden` | Security | **Satisfeito** — REST: `RegisterDefinition_RequiresMcpRegistryWrite_AndAuditsOnSuccess` + `NonPrivilegedIdentity_CannotRemoveDefinition_AssignOrRevoke`; MCP: `AdminMcpRegisterAssignRevoke_ViaMcp_AreRbacGated_AndEnforceRoleAssignmentInvariant` (register/assign/revoke legs) |
| AC-03 | identidade sem nenhum `RoleAssignment` ativo | Owner tenta criar assignment para ela | `409 identity_has_no_role_assignment` | Integration | **Satisfeito** — REST: `McpRegistryEndpointTests.CreateAssignment_ForIdentityWithNoActiveRoleAssignment_Is409`; MCP: `AdminMcpRegisterAssignRevoke_ViaMcp_AreRbacGated_AndEnforceRoleAssignmentInvariant` |
| AC-04 | assignment com um parâmetro `{"secretId": ...}` | self chama render | valor decriptado retornado, `AuditLog SECRET_REVEAL`, nenhum log contém o valor | Security | **Satisfeito** — `McpAssignmentRenderTests.Render_Self_AuditsSecretReveal_WithoutLeakingValueInTheAuditRow` |
...
- [x] testes e evidências definidos — AC-01 a AC-06 (seção 13) e o gap de `RevokedAt` (seção 12) têm teste dedicado, cobrindo REST e MCP onde aplicável (`McpRegistryEndpointTests`, `McpAssignmentRenderTests`, `McpAssignmentValidationTests`, `McpToolsTests`)
- [x] observabilidade definida para este módulo — `mcp_server_renders_total` (M14) além de `AuditLog` (seção 11); a métrica de nível-tools mais ampla (`mcp_calls_total`, módulo 09) segue em aberto lá, não aqui
...
Revisão futura que queira estender este módulo (ex.: validar a UI de "quem está atribuído a este servidor") deve primeiro fechar as questões acima — os gaps de segurança das seções 4/12 já foram corrigidos (revs 3/4) e a cobertura de teste da seção 13 está completa (rev5); só observabilidade e o script de sync (seção 1) seguem em aberto.
